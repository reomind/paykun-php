...
require APPPATH.'libraries/paykun/Payment.php';
require APPPATH.'libraries/paykun/Validator.php';
require APPPATH.'libraries/paykun/Crypto.php';
...
class Pay extends MY_Controller 
{
	...
	function __construct()
	{
		...
		//$this->load->library(array('paykun/payment','paykun/crypto','paykun/validator'));
	...
	function index()
	{
		...
		elseif($gateway_details[0]->type_id == PAYKUN_PAYMENT_GATEWAY ) //PayKun Settings By: Nidhi Soni
		{
        	$MERCHANT_KEY = '';
			...
			foreach($field_values as $value) {						
				...
				{
				    $isLive = false;
				    //print_r('successfully here');
				    if($value->field_key == 'Sandbox_MERCHANT_KEY') {
						$MERCHANT_KEY = $value->field_output_value;
					}
					if($value->field_key == 'Sandbox_API_Secret') {
						$APISECRETKEY = $value->field_output_value;
					}
					if($value->field_key == 'Sandbox_Access_Token') {
						$ACCESSTOKEN = $value->field_output_value;
					}
				}
				else
				{
					...
					if($value->field_key == 'Live_API_Secret') {
						$APISECRETKEY = $value->field_output_value;
					}
					if($value->field_key == 'Live_Access_Token') {
						$ACCESSTOKEN = $value->field_output_value;
					}
				}
			}
			...
			if(!empty($user_details))
			{
				$fname          = $user_details[0]->first_name . ' ' . $user_details[0]->last_name;
				$email          = $user_details[0]->email;
				$contact        = $user_details[0]->phone;
				$country        = $user_details[0]->country;
				$state          = '';
				$city           = $user_details[0]->city;	
				$postalcode     = $user_details[0]->pin_code;
				$address        = $user_details[0]->land_mark;
			}
			...
			$obj = new \Paykun\Checkout\Payment($MERCHANT_KEY, $ACCESSTOKEN, $APISECRETKEY, $isLive, true);   //$APISECRETKEY is encryptionKey
		
		    //	$obj = new \Paykun\Checkout\Payment($MERCHANT_KEY, $ACCESSTOKEN, '', false, true); pls open paykun dashbaord ok
			
			//new
			$successUrl = base_url() . 'pay/payment_success';
			$failUrl = base_url() . 'pay/payment_cancel';
			
            $microtime = str_replace('.', '', microtime(true));
		    $orderId = substr($microtime, 0, 14);
		
			// Initializing Order
			$res = $obj->initOrder($orderId, $product_name,  $amount, $successUrl,  $failUrl, 'INR');
			...
			$obj->addBillingAddress($country, $state, $city, $postalcode, $address);
			// Add Billing Address
	...
	function payment_success()
	{
		...
		if($this->session->userdata('course_purchase_data') && $this->session->userdata('is_valid_request')) {
			...
			$field_values = $this->db->get_where('system_settings_fields',array('type_id' => $gateway_details[0]->type_id))->result();
			...
			elseif($gateway_details[0]->type_id == PAYKUN_PAYMENT_GATEWAY){
			    //validate input 
			    $MERCHANT_KEY = '';
			$ACCESSTOKEN = $APISECRETKEY = '';
			foreach($field_values as $value) {
    ...
    if($response['status'] && $response['data']['transaction']['status'] == "Success") {
			    ...
			    $input_data['paid_date']      	= date('Y-m-d H:i:s');
				$input_data['transaction_id']   = $_REQUEST['payment-id'];
				$input_data['paid_amount']   	= $input_data['amount'];
				$input_data['payer_email']      = $this->session->userdata('email');
				$input_data['payer_name']      	= $this->session->userdata('first_name')." ".$this->session->userdata('last_name');
				$input_data['payment_status']   = "Completed";
